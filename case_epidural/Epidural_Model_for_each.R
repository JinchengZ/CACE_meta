...
jags.m_foreach <- jags.model(file="epidural_m_foreach.txt", data=data, inits=init.jags, n.chains=n.chains, n.adapt=n.adapt)
...
write.table(summary(samps.m_foreach$samples)[[1]], paste("outeach1_",i,".txt",sep=""),sep="\t")
write.table(summary(samps.m_foreach$samples)[[2]], paste("outeach2_",i,".txt",sep=""),sep="\t")
...
for (i in 1:n){
  foreach_CACE[i, ] <- cbind(read.table(paste("outeach2_",i,".txt",sep=""))[1, 1], # lower
                             read.table(paste("outeach1_",i,".txt",sep=""))[1, 1], # mean
                             read.table(paste("outeach2_",i,".txt",sep=""))[1, 3], # median
                             read.table(paste("outeach2_",i,".txt",sep=""))[1, 5], # upper
                             read.table(paste("outeach1_",i,".txt",sep=""))[1, 2]) # SD
}
...
library(metafor)
meta1 <- rma(yi=Mean, vi=SD^2, data=data, method = "REML")
meta2 <- rma(yi=Mean, vi=SD^2, data=data, method = "DL")
meta3 <- rma(yi=Mean, vi=SD^2, data=data, method = "FE")

a <- cbind(meta1$beta, meta1$ci.lb, meta1$ci.ub)
b <- cbind(meta2$beta, meta2$ci.lb, meta2$ci.ub)
c <- cbind(meta3$beta, meta3$ci.lb, meta3$ci.ub)
out <- as.data.frame(rbind(a, b, c))
colnames(out)<-c("Mean", "Lower", "Upper")
rownames(out)<-c("CACE_REML", "CACE_DL", "CACE_FE")
write.table(out, "outputs/CACEout.txt", sep="\t", row.names = F)
