...
params <- c("CACE", "u1out", "v1out", "s1out", "b1out", "pic", "pin", "pia", "sigma_z1", "CACEbystudy") 
...
################# rjags code #########################################################
jags.m2e <- jags.model(file="MRFIT_m2e.txt", data=data, inits=Inits, n.chains=2, n.adapt=2000)
update(jags.m2e, n.iter=5000) # burn in
samps.m2e <- coda.samples.dic(jags.m2e, variable.names=params, n.iter=20000, thin=5)
